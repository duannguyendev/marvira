...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const image =
    ...
    `${SITE.url}${ARTICLE_PLACEHOLDER_IMAGE}`;
...
export default async function ExploreArticlePage({
  ...
  const coverImage =
    resolveArticleImage(article.coverImage) || ARTICLE_PLACEHOLDER_IMAGE;
  const place = article.city
    ? `${article.placeName} · ${article.city}`
    : article.placeName;
  const appDeepLink = article.event
    ? `${SITE.deepLinkScheme}://e/${article.event.id}`
    : null;
