...
const CATEGORY_OPTIONS: Array<{ value: 'all' | FeedbackCategory; label: string }> = [
...
export default function FeedbackPage() {
  ...
  useEffect(() => {
    setPage(1);
  }, [debouncedSearch, pageSize, statusFilter, categoryFilter, sourceFilter, dateFrom, dateTo]);
                ...
                <TableBody>
                  {items.map((item) => (
                    <TableRow key={item.id} className="cursor-pointer hover:bg-muted/50">
                      <TableCell>
                        <Link href={`/dashboard/feedback/${item.id}`} className="block">
                      ...
                      <TableCell>{item.source === FeedbackSource.WEB ? 'Website' : 'Mobile'}</TableCell>
                      <TableCell>
                        <span
